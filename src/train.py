...
def arg_parse():
    ...
    parser.add_argument("--ls_lambda", type=float, help='lambda to regularize the distance to 1 in w optimization', default=1)
    parser.add_argument("--lw_lambda", type=float, help='lambda to regularize the distance to 1 in beta optimization', default=0.005)

    parser.add_argument("--dir_name", type=str, help='specify the directory name', default="debug")
    parser.add_argument("--dataset", type=str, help='specify the dataset', default='CSBM')
    ...
    parser.add_argument("--src_name", type=str, help='specify for the source dataset name',default='US')
    parser.add_argument("--tgt_name", type=str, help='specify for the target dataset name',default='CN')
...
def train(source_dataset, target_dataset, args):
    ...
    # build optimizers
    scheduler, opt = utils.build_optimizer(args, list(model.parameters()))
    ...
    for epoch in range(args.epochs):
        model.train()
        rw_done = train_one_epoch(model, source_dataset, target_dataset, args, opt, scheduler, epoch)
        rw_rec += rw_done
...
def main():
    ...
    args.dir_name = args.dataset + "_" + src_name + "_" + tgt_name + "_EW" + str(args.edge_rw) + \
