...
class GNN(torch.nn.Module):
    def __init__(self, input_dim, output_dim, args):
        ...
        if args.backbone == 'GCN':
            self.prop_input = GCN_reweight(input_dim, args.gnn_dim, args.pooling)
            self.prop_hidden = GCN_reweight(args.gnn_dim, args.gnn_dim, args.pooling)
        elif args.backbone == 'GS':
            self.prop_input = GS_reweight(input_dim, args.gnn_dim, args.pooling)
            self.prop_hidden = GS_reweight(args.gnn_dim, args.gnn_dim, args.pooling)
...
class GNN_dc(torch.nn.Module):
    # domain classifier after the GNN embedding
    def __init__(self, input_dim, out_dim, args):
        super(GNN_dc, self).__init__()

        self.dc = nn.ModuleList()
        if args.dc_layers == 1:
            self.dc.append(nn.Linear(input_dim, out_dim))
        else:
            self.dc.append(nn.Linear(input_dim, args.dc_dim))
            for l in range(args.dc_layers - 2):
                self.dc.append(nn.Linear(args.dc_dim, args.dc_dim))
            self.dc.append(nn.Linear(args.dc_dim, out_dim))
        self.dropout = args.dropout
